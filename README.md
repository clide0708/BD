...
SELECT CONCAT('TRUNCATE TABLE ', table_name, ';') AS stmt FROM information_schema.tables WHERE table_schema = 'bd_tcc' AND table_name NOT IN ('recuperacao_senha', 'exercicios', 'devs', 'exercadaptados', 'videos', 'traducoes_alimentos', 'pagamentos', 'planos') AND table_type = 'BASE TABLE';
